...
# -----------------------------------------------------------------------------
# Helper functions for tensor-based computational memory
# -----------------------------------------------------------------------------
...
def _batch_tensor_memory_mb(batch) -> float:
    total = 0
...
# -----------------------------------------------------------------------------
# Run summary for RSS-based trainers
# -----------------------------------------------------------------------------
        ...
        while True:
            ...
            try:
                with profiler.phase("forward", force_gc=False):
                    ...
                    has_train = hasattr(batch, "train_mask") and int(batch.train_mask.sum()) > 0
                    if has_train:
...
def train_clustergcn(
    ...
    try:
        ...
        raise RuntimeError(
            f"ClusterGCN setup failed. This is usually a dependency/platform issue on Windows. "
            f"Original error: {e}"
        ) from e
...
# -----------------------------------------------------------------------------
# Computational-memory trainers
# -----------------------------------------------------------------------------

def train_graphsage_computational_memory(
    model,
    data,
    epochs: int = 20,
    lr: float = 0.01,
    ...
    for epoch in range(1, epochs + 1):
        ...
        act_mb = bytes_to_mb(tensor_nbytes(out))
    ...
    print()
    print("=" * 72)
    print("COMPUTATIONAL MEMORY SUMMARY (tensor-based estimate)")
    print("=" * 72)
...
def train_graphsaint_computational_memory(
    model,
    data,
    epochs: int = 20,
    lr: float = 0.01,
    ...
    for epoch in range(1, epochs + 1):
        ...
        epoch_forward_peak_mb = 0.0
        epoch_backward_peak_mb = 0.0
        epoch_step_peak_mb = 0.0
        ...
        for batch in loader:
            ...
            param_mb = bytes_to_mb(module_param_nbytes(model))
            input_mb = _batch_tensor_memory_mb(batch)
            act_mb = bytes_to_mb(tensor_nbytes(out))
            loss_mb = bytes_to_mb(tensor_nbytes(loss))

            forward_comp_mb = param_mb + input_mb + act_mb + loss_mb
            epoch_forward_peak_mb = max(epoch_forward_peak_mb, forward_comp_mb)

            loss.backward()
            grad_mb = bytes_to_mb(module_grad_nbytes(model))
            backward_comp_mb = param_mb + input_mb + act_mb + grad_mb
            epoch_backward_peak_mb = max(epoch_backward_peak_mb, backward_comp_mb)

            optimizer.step()
            opt_state_mb = bytes_to_mb(optimizer_state_nbytes(optimizer))
            step_comp_mb = param_mb + grad_mb + opt_state_mb
            epoch_step_peak_mb = max(epoch_step_peak_mb, step_comp_mb)
        ...
        memory_rows.append({
            "epoch": epoch,
            "forward_comp_peak_mb": epoch_forward_peak_mb,
            "backward_comp_peak_mb": epoch_backward_peak_mb,
            "step_comp_peak_mb": epoch_step_peak_mb,
        })
        ...
        if epoch == 1 or epoch % 20 == 0 or epoch == epochs:
            print(
                ...
                f"test_macro_f1={metrics['test_macro_f1']:.4f} | "
                f"epoch_time={epoch_time:.4f}s | "
                f"forward_comp_peak={epoch_forward_peak_mb:.2f} MB"
            )
...
def train_clustergcn_computational_memory(
    model,
    data,
    epochs: int = 20,
    lr: float = 0.01,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in loader:
            ...
            # Supports both normal forward and optional activation-returning forward
            try:
                out, activations = model(batch.x, batch.edge_index, return_activations=True)
            ...
            param_mb = bytes_to_mb(module_param_nbytes(model))
            input_mb = _batch_tensor_memory_mb(batch)
            logits_loss_mb = bytes_to_mb(tensor_nbytes(loss))

            forward_comp_mb = param_mb + input_mb + act_mb + logits_loss_mb
            epoch_forward_peak_mb = max(epoch_forward_peak_mb, forward_comp_mb)
